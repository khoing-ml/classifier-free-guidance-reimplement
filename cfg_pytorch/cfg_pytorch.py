...
from cfg_pytorch.attend import Attend
    ...
    def inner(
        ...
        **kwargs
        ...
        def fn_maybe_with_text(self, *args, **kwargs):
            if auto_handle_text_condition: # handling the text condition automatically 
                texts = kwargs.pop('texts', None) # get the texts if exists in params
                text_embeds = kwargs.pop('text_embeds', None) # also the embedded text if available
...
class Attention(Module):
    def __init__ (self, dim, dim_head= 64, heads= 8, dim_context=None, norm_context=False, num_null_kv=0, flash=False):
        super().__init__()
        self.heads = heads
        ...
        self.null_kv = nn.Parameter(torch.rand(2, num_null_kv, dim_head))
